...
class LokersearchSpider(scrapy.Spider):
    name                 = 'lokersearch-500'
    allowed_domains      = ['loker.id']
    start_urls           = ['https://www.loker.id/cari-lowongan-kerja/page/500?q&lokasi=0']
    
    def parse(self, response):    
        page_number = response.meta.get('page_number') or 500          
        ...
        if int(page_number) == 937: 
            page_number = 937
        elif int(page_number) >= 500: 
