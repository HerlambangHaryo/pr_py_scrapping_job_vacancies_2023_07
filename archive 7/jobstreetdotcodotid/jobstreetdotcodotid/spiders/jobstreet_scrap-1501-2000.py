...
class JobstreetsearchSpider(scrapy.Spider):
    name                 = 'jobstreetsearch-2000'
    allowed_domains      = ['jobstreet.co.id']
    start_urls           = ['https://www.jobstreet.co.id/en/job-search/job-vacancy/1501/?src=16&srcr=16&ojs=1']
    
    def parse(self, response):    
        page_number = response.meta.get('page_number') or 1501   
        
        if int(page_number) == 1572: 
            page_number = 1572
        elif int(page_number) >= 1501: 
